...
from nova.db.deployment_models import (
    ...
    ActiveTask, TaskStatus, TaskCheckpoint, ScheduledJob, NotificationLog
)
...
class QueueManager:
    ...
    def add_to_queue(
        ...
        with self._lock:
            ...
            try:
                ...
                if self._notification_callback and self._is_destructive_action(deployment_type):
    ...
    def get_active_worker_count(self) -> int:
        ...
        session = self._get_session()
        try:
            return session.query(ActiveTask).filter(
        ...
        try:
            ...
            if item.requires_state_pause:
                ...
                if worker_count > 0:
                    ...
                    return False, f"Waiting for {worker_count} active worker(s) to complete"
    ...
    def retry_failed_items(self) -> int:
        ...
        session = self._get_session()
        try:
            failed_items = session.query(DeploymentQueue).filter(
                and_(
