    ...
    def start(self):
        ...
        with self._lock:
            ...
            self._process_thread = threading.Thread(
                target=self._process_queue_loop, name="DeploymentProcessor", daemon=True
            )
            self._process_thread.start()

            # Start scheduler thread
            self._scheduler_thread = threading.Thread(
                target=self._scheduler_loop, name="JobScheduler", daemon=True
            )
    ...
    def _process_next_deployment(self):
        ...
        if success:
            ...
            self.queue_manager.update_status(
                queue_item.id, QueueStatus.FAILED, error_message=error_msg
            )
    ...
    def _execute_scheduled_job(self, job, session):
        ...
        if job.auto_resume and job.last_checkpoint_id:
            ...
            checkpoint = (
                ...
                .filter(TaskCheckpoint.id == job.last_checkpoint_id)
                .first()
            )
