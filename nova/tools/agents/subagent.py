...
from nova.tools.github.github_tools import push_to_github, pull_latest_changes
...
from nova.tools.core.streaming_utils import (
    _get_telegram_bot,
...
async def run_subagent_task(subagent_id: str, agent: Agent, instruction: str, user_id: str = "system"):
    ...
    async with StreamingContext(
        ...
        try:
            ...
            # Send final result — only if not silent
            if not silent:
                from nova.tools.core.streaming_utils import send_live_update
                await send_live_update(result.replace("🎲", "").strip()[:3500], chat_id)
