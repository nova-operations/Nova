...
class ErrorBusHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if record.name in (
            "nova.tools.core.error_bus",
            "nova.tools.scheduler.scheduler",
            "nova.tools.core.heartbeat",
            "httpx",
            "telegram",
            "openai",
        ):
        ...
        if re.search(
            ...
            logging.getLogger("nova.tools.core.error_bus").warning(
                f"[TOOL-MISS] {msg[:120]}"
            )
            return
...
async def _error_monitor_loop():
    ...
    logger = logging.getLogger("nova.tools.core.error_bus")
    while True:
        try:
            Session = get_session_factory()
            db = Session()
...
def start_error_bus():
    ...
    if not any(isinstance(h, ErrorBusHandler) for h in root_logger.handlers):
        ...
        logging.getLogger("nova.tools.core.error_bus").info(
    ...
    if _error_monitor_task is None or _error_monitor_task.done():
        ...
        logging.getLogger("nova.tools.core.error_bus").info("ErrorBus monitor started.")
