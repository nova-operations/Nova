...
class ContextOptimizer:
    ...
    def __init__(self, model_id: str = None, api_key: str = None):
        self.model_id = model_id or os.getenv("SUBAGENT_MODEL", "google/gemini-2.0-flash-001")
...
Summary:"""
            ...
            summary_agent = Agent(
                model=model,
                instructions=["You are a concise summarizer. Output only the summary, no extra text."],
                markdown=False,
                tools=[],
            )
    ...
    async def optimize(
        ...
        if method == "truncate":
            ...
            optimized_content = f"[Content split into {len(chunks)} chunks]\n\n" + "\n\n---\n\n".join(
                f"CHUNK {i+1}/{len(chunks)}:\n{c}" for i, c in enumerate(chunks[:10])  # Limit to 10 chunks
            )
            if len(chunks) > 10:
