...
class MiddleOutTransformer:
    ...
    def apply_middle_out(self, content: str, max_length: int) -> str:
        ...
        omitted = (
            content_len - len(start_section) - len(middle_section) - len(end_section)
        )
    ...
    def transform(self, prompt: str) -> TransformResult:
        ...
        available_for_history = (
            self.max_chars - len(system_prompt) - len(latest_message) - header_size
        )
...
def transform_prompt(
    prompt: str, max_tokens: int = DEFAULT_TOKEN_LIMIT
) -> Tuple[str, bool]:
