...
class TestOptimizeFunction:
    ...
    def test_explicit_methods(self):
        ...
        # Test truncate
        result = asyncio.run(self.optimizer.optimize(content, method="truncate"))
        assert result.method_used == "truncate"
        
        # Test middle-out
        result = asyncio.run(self.optimizer.optimize(content, method="middle-out"))
        assert result.method_used == "middle-out"
        
        # Test chunk
        result = asyncio.run(self.optimizer.optimize(content, method="chunk"))
