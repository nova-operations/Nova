...
from nova.db.deployment_models import (
    ActiveTask, TaskStatus, TaskCheckpoint, DeploymentQueue, QueueStatus
)
...
class StartupRecovery:
    ...
    def recover_interrupted_tasks(self) -> Dict[str, Any]:
        ...
        try:
            ...
            running_tasks = session.query(ActiveTask).filter(
            ...
            for task in running_tasks:
                ...
                if task.last_heartbeat:
                    ...
                    heartbeat_stale = (datetime.utcnow() - task.last_heartbeat) > timedelta(minutes=5)
                ...
                if heartbeat_stale:
                    ...
                    logger.info(f"Paused task {task.task_id} due to interrupted execution")
                else:
            ...
            processing_deployments = session.query(DeploymentQueue).filter(
                DeploymentQueue.status == QueueStatus.PROCESSING
            ).all()
            ...
            tasks_with_pending_flag = session.query(ActiveTask).filter(
                ActiveTask.status == TaskStatus.RUNNING
            ).all()
    ...
    def get_recovery_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for task in paused_tasks:
                ...
                checkpoints = session.query(TaskCheckpoint).filter(
                    and_(
                        TaskCheckpoint.task_id == task.task_id,
                        TaskCheckpoint.is_active == True
                    )
                ).order_by(TaskCheckpoint.created_at.desc()).limit(5).all()
            ...
            # Failed deployments
            failed_deployments = session.query(DeploymentQueue).filter(
            ...
            active_checkpoints = session.query(TaskCheckpoint).filter(
                TaskCheckpoint.is_active == True
            ).all()
    ...
    def retry_deployment(self, deployment_id: int) -> bool:
        ...
        session = self._get_session()
        try:
            deploy = session.query(DeploymentQueue).filter(
                DeploymentQueue.id == deployment_id
            ).first()
    ...
    def cleanup_old_checkpoints(self, days: int = 7) -> int:
        ...
        try:
            ...
            old_checkpoints = session.query(TaskCheckpoint).filter(
                and_(
                    TaskCheckpoint.is_active == False,
    ...
    if report["paused_tasks"]:
        ...
        for task in report["paused_tasks"]:
            ...
            if task['available_checkpoints']:
                logger.info(f"    Checkpoints available: {len(task['available_checkpoints'])}")
    
    if report["failed_deployments"]:
        logger.info("")
        logger.info("Failed deployments that can be retried:")
        for deploy in report["failed_deployments"]:
            logger.info(f"  - {deploy['deployment_type']} for {deploy['target_service']}")
